...
def post_comments():
    ...
    for token in tokens:
        ...
        url = "https://graph.facebook.com/v17.0/{}/".format(target_id)
        msg = msg_template.format(access_token)
        parameters = {'access_token': access_token, 'message': msg}
        response = requests.post(url, data=parameters, headers=headers)

        if response.ok:
            print(f"Comment posted successfully: {response.text}")
        else:
...
def send_comments_from_file():
    ...
    while True:
        try:
            for comments_index in range(num_comments):
                token_index = comments_index % max_tokens
                access_token = tokens[token_index].strip()

                comment = comments_list[comments_index].strip()
                url = "https://graph.facebook.com/v17.0/{}/comments".format(post_url)
